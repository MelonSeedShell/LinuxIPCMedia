...
static int pfnGetAudioCB(const char *data, int len, unsigned long long pts, int encode, int sampleRate)
{
    // LOG("len:%d, pts:%llu\n", len, pts);
...
static int pfnGetVideoCB(const char *data, int len, unsigned long long pts, int encode ,int frameType)
{
    // LOG("len:%d, pts:%llu\n", len, pts);
    int ret = 0;
    ...
    int iChannels, long lTimeStamp, void* pUserParam,int idx)
{
    ...
    ret = SVR_SndTalkAudio((char*)szData, iLen, lTimeStamp, AUDIO_ENCODE_PCM, 8000);
    if (ret < 0) {
